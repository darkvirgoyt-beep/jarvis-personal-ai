  ...
  return (
    <section className="hud-panel p-4 sm:p-5" aria-label="Jarvis virtual workspace">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><p className="hud-label">JARVIS // PRIVATE WORKSPACE</p><h2 className="mt-1 text-base font-semibold text-slate-100">Private files, folders, and code</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">This browser-based Jarvis workspace is active and isolated from your phone or computer. File writes are stored privately only after explicit approval. No deletes or device access are enabled here.</p></div>
        <span className="flex items-center gap-1.5 rounded-sm border border-cyan-300/25 bg-cyan-300/[0.06] px-2 py-1 text-[10px] text-cyan-100"><HardDrive className="size-3" /> BROWSER WORKSPACE</span>
      </div>
      ...
      <div className="mt-3 rounded-sm border border-cyan-300/15 bg-cyan-300/[0.025] p-3">
        <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex gap-2"><MonitorSmartphone className="mt-0.5 size-4 shrink-0 text-cyan-200" /><div><p className="hud-label">OPTIONAL CLOUD COMPUTER</p><p className="mt-1 text-xs leading-5 text-slate-400">No persistent computer is attached to this workspace yet. The active browser workspace can prepare private file, folder, and code proposals; a connected managed computer is required before Jarvis can prepare reviewable browser tasks in a visible session or run and compile software.</p></div></div><span className="rounded-sm border border-amber-300/25 bg-amber-300/[0.06] px-2 py-1 text-[10px] font-semibold text-amber-100">NOT ATTACHED</span></div>
