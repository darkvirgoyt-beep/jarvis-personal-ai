...
import { getCompileRecipePreview, summarizeTerminalAuditDetails } from "@/lib/virgoytTerminalReview";
...
function ReviewedTerminalPanel({ proposals, auditEvents, runnerActive }: {
  proposals: Array<{ id: number; toolKind: string; status: string; payloadJson: string }>;
  auditEvents: Array<{ id: number; eventKind: string; detailsJson: string; createdAt: Date | string }>;
  runnerActive: boolean;
}) {
  const compileProposal = proposals.find((proposal) => proposal.toolKind === "terminal_command" && getCompileRecipePreview(proposal.payloadJson));
  const recipe = compileProposal ? getCompileRecipePreview(compileProposal.payloadJson) : undefined;
  return <section className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(280px,0.85fr)]"><div className="rounded-sm border border-cyan-300/15 bg-black/25 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="hud-label text-cyan-100">REVIEWED TERMINAL</p><h2 className="mt-2 text-lg font-semibold text-white">Real command timeline</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">Shows only persisted proposals and runner events. Jarvis cannot accept an arbitrary shell command from this panel.</p></div><StatusPill value={runnerActive ? "active" : "runner_unconnected"} /></div><div className="mt-4 max-h-72 space-y-2 overflow-y-auto overscroll-contain pr-1">{auditEvents.slice(0, 10).map((event) => <div key={event.id} className="flex gap-3 rounded-sm border border-white/[0.07] bg-slate-950/55 p-3"><span className="mt-1 size-2 shrink-0 rounded-full bg-cyan-300 shadow-[0_0_10px_rgba(103,232,249,0.85)]" /><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><p className="font-mono text-[10px] text-cyan-100">{event.eventKind}</p><span className="text-[10px] text-slate-600">{formatTime(event.createdAt)}</span></div><p className="mt-1 break-words text-xs leading-5 text-slate-400">{summarizeTerminalAuditDetails(event.detailsJson)}</p></div></div>)}{!auditEvents.length && <p className="rounded-sm border border-dashed border-white/15 p-4 text-xs leading-5 text-slate-500">No runner evidence yet. Stage and approve a fixed recipe; a paired runner must claim it before execution can be reported.</p>}</div></div><div className="rounded-sm border border-fuchsia-300/20 bg-fuchsia-400/[0.035] p-4"><p className="hud-label text-fuchsia-100">DIFF REVIEW</p><h2 className="mt-2 text-lg font-semibold text-white">Persisted recipe preview</h2>{recipe ? <><div className="mt-4 rounded-sm border border-white/10 bg-slate-950/75 p-3 font-mono text-[10px] leading-5 text-slate-300"><p className="text-fuchsia-200">--- approval-required recipe</p><p className="text-cyan-100">+++ {recipe.workspacePath} · {recipe.environment}</p>{recipe.steps.map((step) => <p key={step} className="text-emerald-100">+ {step}</p>)}{recipe.artifactHints.map((hint) => <p key={hint} className="text-slate-500"># artifact {hint}</p>)}</div><div className="mt-3 flex items-center justify-between gap-2"><StatusPill value={compileProposal?.status ?? "pending"} /><span className="text-[10px] text-slate-600">proposal #{compileProposal?.id}</span></div></> : <div className="mt-4 rounded-sm border border-dashed border-fuchsia-300/20 p-4 text-xs leading-5 text-slate-500">No fixed compile recipe is staged. File/code diffs appear only when a persisted reviewed proposal contains real artifact data; Jarvis does not invent a changed-file result.</div>}</div></section>;
}
...
export default function VirgoYTAgent() {
  ...
  const [planningModel, setPlanningModel] = useState<"nvidia/nemotron-3-ultra-550b-a55b" | "anthropic/claude-fable-5">("anthropic/claude-fable-5");
  ...
  const handleCreateRun = async () => {
    ...
    try {
      const run = await createRun.mutateAsync({ projectId: activeProjectId, agent: selectedAgent, provider: "openrouter", modelId: planningModel, request: request.trim() });
