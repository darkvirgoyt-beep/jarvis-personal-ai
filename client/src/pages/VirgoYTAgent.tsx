...
import { VirgoYTSettingsPanel } from "@/components/VirgoYTSettingsPanel";
...
import { AlertTriangle, ArrowLeft, Bot, Check, ChevronRight, Code2, FileCode2, FolderKanban, HardDrive, History, LockKeyhole, Play, Plus, Radio, Rocket, ScrollText, Settings2, ShieldCheck, TerminalSquare, Wrench, X } from "lucide-react";
...
export default function VirgoYTAgent() {
  ...
  const credentialVaultQuery = trpc.virgoyt.providers.credentialVault.useQuery(undefined, { enabled: Boolean(user) });
  ...
  const handleCreateProvider = async () => {
    ...
    try {
      const apiKey = providerApiKey.trim() || undefined;
      if (apiKey && !credentialVaultQuery.data?.configured) return setNotice("This deployment does not yet have its server-only VirgoYT credential vault configured.");
      await createProvider.mutateAsync({ label: providerLabel.trim(), provider: "compatible", endpoint: providerEndpoint.trim() || null, defaultModel: null, apiKey });
      setProviderLabel("");
      setProviderEndpoint("");
      setProviderApiKey("");
      setNotice(apiKey ? "Provider profile saved. Its credential was encrypted server-side and is not shown again." : "Provider metadata saved without a credential.");
  ...
  return <main className="min-h-screen bg-[#05080f] text-slate-100"><div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_85%_0%,rgba(217,70,239,0.13),transparent_30%),radial-gradient(circle_at_12%_18%,rgba(34,211,238,0.1),transparent_27%)]" />
    <header className="relative z-10 border-b border-cyan-300/15 bg-slate-950/80 backdrop-blur"><div className="mx-auto flex max-w-[1700px] items-center justify-between gap-4 px-4 py-3 sm:px-6"><div className="flex min-w-0 items-center gap-3"><Link href="/" className="flex size-9 shrink-0 items-center justify-center rounded-sm border border-cyan-300/30 bg-cyan-300/10 text-cyan-100 hover:bg-cyan-300/20" aria-label="Back to Jarvis chat"><ArrowLeft className="size-4" /></Link><div className="min-w-0"><p className="hud-label truncate text-cyan-100">JARVIS // VIRGOYT AGENT WORKSPACE</p><p className="truncate text-sm text-slate-400">{activeProject?.name ?? "Private agent planning, connected to Jarvis"}</p></div></div><div className="hidden items-center gap-2 text-[10px] font-medium tracking-[0.14em] sm:flex"><Radio className="size-3 text-cyan-200" /><span className="text-cyan-100">SYNC 6S</span><span className="rounded-full border border-emerald-300/25 bg-emerald-300/10 px-2 py-1 text-emerald-100">PRIVATE SESSION</span></div></div></header>

    <div className="relative z-10 mx-auto grid max-w-[1700px] gap-4 p-4 lg:grid-cols-[220px_minmax(0,1fr)_340px] lg:p-6">
      <aside className="rounded-sm border border-white/10 bg-slate-950/80 p-2 lg:sticky lg:top-6 lg:h-[calc(100vh-6rem)]"><div className="mb-3 border-b border-white/[0.07] px-3 py-3"><p className="text-xs font-semibold text-white">{user.name || "Private operator"}</p><p className="mt-1 truncate text-[11px] text-slate-500">{user.email || "Authenticated workspace"}</p></div><nav aria-label="VirgoYT workspace areas" className="grid grid-cols-2 gap-1 lg:grid-cols-1">{tabs.map((item) => { const Icon = item.icon; return <button key={item.id} type="button" onClick={() => setTab(item.id)} className={cn("flex min-h-11 items-center justify-start gap-2 rounded-sm px-3 py-2 text-left text-xs transition", tab === item.id ? "bg-cyan-300/10 text-cyan-100" : "text-slate-500 hover:bg-white/[0.04] hover:text-slate-200")}><Icon className="size-4" /><span>{item.label}</span></button>; })}</nav><div className="mt-4 rounded-sm border border-amber-300/15 bg-amber-300/[0.035] p-3"><ShieldCheck className="size-4 text-amber-200" /><p className="mt-2 text-[11px] font-semibold text-amber-100">Approval-first tools</p><p className="mt-1 text-[10px] leading-4 text-slate-500">No files, commands, navigation, Git, or deployments execute from a plan alone.</p></div></aside>
      ...
      <section className="min-w-0 rounded-sm border border-white/10 bg-slate-950/80 p-4 shadow-[0_0_45px_rgba(34,211,238,0.04)] sm:p-6">
        ...
        {tab === "settings" && <VirgoYTSettingsPanel providerLabel={providerLabel} providerEndpoint={providerEndpoint} providerApiKey={providerApiKey} vaultConfigured={Boolean(credentialVaultQuery.data?.configured)} providerProfiles={providersQuery.data ?? []} runnerName={runnerName} runnerType={runnerType} runners={runnersQuery.data ?? []} createPending={createProvider.isPending} runnerPending={registerRunner.isPending} onProviderLabelChange={setProviderLabel} onProviderEndpointChange={setProviderEndpoint} onProviderApiKeyChange={setProviderApiKey} onCreateProvider={() => void handleCreateProvider()} onRunnerNameChange={setRunnerName} onRunnerTypeChange={setRunnerType} onRegisterRunner={() => void handleRegisterRunner()} />}
      </section>
