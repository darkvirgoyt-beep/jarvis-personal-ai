...
import { JarvisGlyph } from "@/components/JarvisGlyph";
import { JarvisMemoryDashboard, type JarvisMemoryCategory } from "@/components/JarvisMemoryDashboard";
...
import { Activity, BellRing, Bot, BrainCircuit, Check, ChevronRight, CircleDot, Code2, Command, Cpu, Database, FileText, Layers3, LockKeyhole, Mic, Orbit, Plus, Search, Settings2, ShieldCheck, Sparkles, TerminalSquare, Volume2, X } from "lucide-react";
      ...
      <div className="jarvis-workspace-shell mx-auto flex w-full max-w-[1680px] flex-col gap-3 md:flex-row">
        ...
        <div className="jarvis-workspace-content min-w-0 flex-1">
          <header className="mb-3 flex min-h-14 w-full items-center justify-between gap-3 rounded-xl border border-white/10 bg-slate-950/55 px-3 py-2.5 backdrop-blur-xl sm:px-4">
            <div className="flex min-w-0 items-center gap-3"><JarvisMobileWorkspaceTrigger onOpen={() => setRailOpen(true)} /><div className="hidden sm:block"><JarvisGlyph size={32} status={coreState === "thinking" || coreState === "speaking" ? "active" : "ready"} /></div><div className="min-w-0"><p className="hud-label">JARVIS WORKSPACE</p><p className="mt-1 truncate text-xs text-slate-500">{activeMode === "chats" ? "Private conversations and voice commands" : activeMode === "memory" ? "Long-term memory, on your terms" : activeMode === "projects" ? "Private files and project workspaces" : activeMode === "builder" ? "Plan websites and applications" : activeMode === "integrations" ? "Reviewable external handoffs" : "Personal assistant controls"}</p></div></div>
            <div className="flex items-center gap-2"><div className="hidden items-center gap-2 rounded-lg border border-white/10 bg-black/20 px-3 py-2 sm:flex"><StatusDot state={coreState} /><span className="text-[11px] text-slate-400">{statusCopy[coreState]}</span></div><div aria-label="Secure browser session active" title="Your Jarvis session is stored securely on this browser. Sign out on shared devices." className="hidden items-center gap-1.5 rounded-lg border border-emerald-300/20 bg-emerald-300/[0.06] px-2.5 py-2 text-[10px] font-medium text-emerald-100 lg:flex"><ShieldCheck className="size-3.5" aria-hidden /> Session active</div><Link href="/agent" className="hidden items-center gap-1.5 rounded-lg border border-fuchsia-400/25 bg-fuchsia-400/[0.08] px-3 py-2 text-xs font-medium text-fuchsia-100 transition hover:bg-fuchsia-400/15 md:flex"><Bot className="size-3.5" /> VirgoYT Agent</Link><button onClick={startNewChat} className="hidden items-center gap-1.5 rounded-lg border border-cyan-300/25 bg-cyan-300/10 px-3 py-2 text-xs font-medium text-cyan-100 transition hover:bg-cyan-300/15 sm:flex"><Plus className="size-3.5" /> New chat</button></div>
          </header>
      ...
      <div aria-label="Chats" className={cn("jarvis-chat-cockpit grid w-full grid-cols-1 gap-3 2xl:grid-cols-[245px_minmax(0,1fr)_285px]", activeMode !== "chats" && "hidden")}>
        ...
        <section className="flex min-w-0 flex-col gap-3">
          ...
          <div className="jarvis-chat-stage grid min-w-0 gap-3 lg:grid-cols-[minmax(300px,0.85fr)_minmax(360px,1.15fr)]">
            ...
            <HudPanel className="sticky top-2 flex h-[min(74dvh,760px)] min-h-[490px] flex-col overflow-hidden sm:top-3 lg:h-[min(78dvh,800px)]">
