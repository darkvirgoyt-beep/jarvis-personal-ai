...
function preference(row: Row, userId: number) {
  return { id: number(row.id), userId, model: row.model, personality: row.personality, voiceEnabled: row.voice_enabled ? 1 : 0, voiceName: row.voice_name, continuousMode: row.continuous_mode ? 1 : 0, contextualSuggestions: row.contextual_suggestions ? 1 : 0, speechRate: row.speech_rate, privacyMode: row.privacy_mode, durableMemoryEnabled: row.durable_memory_enabled ? 1 : 0, visualMode: row.visual_mode, pluginSettings: row.plugin_settings, createdAt: date(row.created_at)!, updatedAt: date(row.updated_at)! };
}
...
export async function updateJarvisPreferences(input: { userId: number; model?: string; personality?: "balanced" | "concise" | "strategic" | "creative"; voiceEnabled?: number; voiceName?: string | null; continuousMode?: number; contextualSuggestions?: number; speechRate?: number; privacyMode?: "standard" | "minimal"; durableMemoryEnabled?: number; visualMode?: "hud" | "reduced_motion"; pluginSettings?: string | null }) {
  const runtime = requireSupabaseRuntimeClient(); const openId = await openIdForUserId(input.userId);
  const values = compact({ model: input.model, personality: input.personality, voice_enabled: input.voiceEnabled === undefined ? undefined : Boolean(input.voiceEnabled), voice_name: input.voiceName, continuous_mode: input.continuousMode === undefined ? undefined : Boolean(input.continuousMode), contextual_suggestions: input.contextualSuggestions === undefined ? undefined : Boolean(input.contextualSuggestions), speech_rate: input.speechRate, privacy_mode: input.privacyMode, durable_memory_enabled: input.durableMemoryEnabled === undefined ? undefined : Boolean(input.durableMemoryEnabled), visual_mode: input.visualMode, plugin_settings: input.pluginSettings, updated_at: iso(new Date()) });
